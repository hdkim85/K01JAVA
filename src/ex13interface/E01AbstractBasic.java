...
abstract class AbsClass1{
	//int형으로 표현할 수 있는 최대값을 반환하는 Wrapper 클래스
...
/*
 	부모가 가진 추상메소드를 그대로 상속 받게 되므로 에러가 나지
 	않으려면 해당클래스도 abstract로 선언하고나, 부모의
 	abstract메소드를 오버라이딩 처리해야한다.
 */

class absClassChild2 extends absClass2{
	@Override
	void absMethod(int number) {
		System.out.println("부모 클래스에서 오버라이딩한 메소드로 없을 땐 에러 발생함");
...
//		AbsClass1 absClass1 = new AbsClass1();에러
		
		
		//추상클래스를 상속한하위클래스는 객체생성이 가능하다.
