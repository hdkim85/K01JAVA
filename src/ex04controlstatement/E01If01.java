...
*/
		...
		/*
		 if(조건식); => 조건식에 상관없이 다음 문장이 실행되는 형태로
		 	세미콜론은 문장의 끝은 나타내므로 사용에 주의해야 한다.
		 */
		...
		/*
		 System.in.read()
		 	:사용자에게 하나의 문자를 입력 받을 때 사용하는 메소드로
		...
		{
			System.out.println((char)asciiCode + "는 숫자입니다.");
		}
		else if(asciiCode <= 122 && asciiCode >= 65)
		{
			System.out.println((char)asciiCode + "는 문자입니다.");
		}
		else
		{
			System.out.println((char)asciiCode + "는 숫자나 문자가 아닙니다.");
		}
		/*
		 아스키코드 값을 모를 때 구현방법
		 char형 문자는 메모리에 저장 시 아스키코드값으로 저장되므로 아래처럼
		 최고연산이 가능하다. 또 한 산술연산도 가능하다.
		 */
