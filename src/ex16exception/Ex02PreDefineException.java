...
public class Ex02PreDefineException {
	...
	public static void main(String[] args) {
		...
		try {
			...
			*/
			...
			System.out.println("앗 이 방은 : " + intArr[2]);
			...
			/*
			 ...
			 printStackTrace() : 예외가 발생할 때의 에러메세지를
			 	그대로 콘솔에 출력해줌. 개발 시 훨씬 더 사용빈도 높음
			 */
		...
		:숫자형식의 문자열을 int로 반환하는경우 해당 문자열이
		숫자형식이 아니면 발생되는 예외
		 */
		...
		try {
			...
			System.out.println("나이를 문자형태로 입력하면 안돼");
			System.out.println("예외 메세지 : " +e.getMessage());
			e.printStackTrace();
		}
		...
		System.out.println("today=" + toDay);/*
			toDay 참조변수가 가지고 있는 null값을 toString()메소드를 통해
			String형태로 변환되어 출력된다. 예외는 발생하지 않는다.
			*/
		try {
			/*
	...
	/*
	String 객체가 매개변수로 전달되면서 Object로 형변환되기 떄문에,
	여기서는 상속관계가 확인대ㅓ  true를 반환
	 */
